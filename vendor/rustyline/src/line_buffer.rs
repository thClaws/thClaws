...
impl LineBuffer {
    ...
    fn prev_pos(&self, n: RepeatCount) -> Option<usize> {
        ...
        while remaining > 0 {
            ...
            let last_char = self.buf[..pos].chars().rev().next()?;
            let cp = last_char as u32;
            // Thai non-spacing combining marks (สระ/วรรณยุกต์): delete one
            // codepoint at a time instead of the whole Extended Grapheme Cluster.
            let is_thai_combining =
                cp == 0x0E31 || (0x0E34..=0x0E3A).contains(&cp) || (0x0E47..=0x0E4E).contains(&cp);
            if is_thai_combining {
                pos -= last_char.len_utf8();
            } else {
                pos = self.buf[..pos]
                    .grapheme_indices(true)
                    .rev()
...
mod test {
    ...
    #[test]
    fn prev_pos_thai_combining() {
        // "กิ้" = U+0E01 (3B) + U+0E34 (3B) + U+0E49 (3B) = 9 bytes
        // Backspace should delete one codepoint at a time, not the whole cluster.
        let s = LineBuffer::init("กิ้", 9);
        assert_eq!(9, s.len());
        assert_eq!(Some(6), s.prev_pos(1)); // ลบ ้ (วรรณยุกต์) → pos 6
        let s = LineBuffer::init("กิ้", 6);
        assert_eq!(Some(3), s.prev_pos(1)); // ลบ ิ (สระ) → pos 3
        let s = LineBuffer::init("กิ้", 3);
        assert_eq!(Some(0), s.prev_pos(1)); // ลบ ก (พยัญชนะ grapheme) → pos 0

        // n=2 from pos 9: ลบ ้ แล้ว ิ → pos 3
        let s = LineBuffer::init("กิ้", 9);
        assert_eq!(Some(3), s.prev_pos(2));

        // Latin after Thai: "กa" — ก is non-combining, delete by grapheme
        let s = LineBuffer::init("กa", 4); // ก(3B) + a(1B) = 4B
        assert_eq!(Some(3), s.prev_pos(1)); // ลบ a
    }
